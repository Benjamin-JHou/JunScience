...
import { mockDefaultArtifacts } from '../../../data/mockResearch';
...
export const DataAnalysisTool: ToolDefinition<DataAnalysisInput> = {
  name: 'data_analysis',
  ...
  async execute(input: DataAnalysisInput, context: ToolContext): Promise<ToolExecutionResult> {
    context.reportProgress(`Loading dataset "${input.datasetIdOrPath}" (14,200 cells x 24,180 features)...`, 20);
    context.reportProgress(`Running quality control: filtering low-complexity cells (<8% mitochondrial reads)...`, 40);
    context.reportProgress(`Executing Seurat v5 SCTransform with cell-cycle regression...`, 70);
    context.reportProgress(`Running non-parametric Wilcoxon test for SLE Flare vs Healthy Controls...`, 90);

    const tableArtifact = mockDefaultArtifacts.find((a) => a.type === 'table') || {
    ...
    return {
      ...
      execution: {
        ...
        logs: [
          ...
          `Statistical Method: Seurat v5 SCTransform + Wilcoxon rank-sum test`,
