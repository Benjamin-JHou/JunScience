...
export const UniProtTool: ToolDefinition<UniProtInput> = {
  ...
  async execute(input: UniProtInput, context: ToolContext): Promise<ToolExecutionResult> {
    ...
    context.reportProgress(`Resolved UniProt entry: ${proteinData.entryName} (${proteinData.primaryAccession})`, 70);
    context.reportProgress(`Extracted 4 structural domains and catalytic annotations`, 100);
    ...
    return {
      ...
      execution: {
        ...
        logs: [
          ...
          `Disease association confirmed: ${proteinData.diseaseInvolvement}`,
        ],
      },
    };
  },
