...
export class SkillRegistry {
  ...
  private initDefaultSkills(): void {
    ...
    // --- Tier 1: High-Value Biology & Chemistry Skills ---
    this.register({
      name: 'scanpy',
  ...
  public listByTier(tier: 'tier0' | 'tier1' | 'tier2'): SkillMetadata[] {
    return this.list().filter((s) => s.tier === tier);
  }

  public discover(query: string, maxResults: number = 3): SkillMetadata[] {
    ...
    const scored = this.list().map((skill) => {
      ...
      queryTokens.forEach((token) => {
        ...
        if (skill.keywords.some((kw) => kw.includes(token))) score += 5;
      });

      if (skill.tier === 'tier0') score += 1; // Bias slightly to core foundational skills
